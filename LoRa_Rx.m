function [message, symbols_hdr_fec, symbols_pld_fin] = LoRa_Rx(signal,Bandwidth,SF,Coherece,Fs,df,varargin)
...
try
    ...
    [message_full, symbols_hdr_fec, symbols_pld_fin] = LoRa_Decode_Full(symbols_message,SF);
    %message = message_full;
    message             = message_full(8:4 + message_full(1) - 2) ;

catch ME
    %message             = ME.identifier; %NaN ;
    message             = NaN ;
end
end
...
function [message_full,CR_pld,pld_length,CRC_pld, symbols_hdr_fec, symbols_pld_fin] = LoRa_Decode_Full(symbols_message,SF)
...
if CR > 2 && CR <= 4 % detection and correction
    ...
    for ctr = 0 : n - 1
        ...
        %if deocded(ctr+1) ~= bitor(bitsll(s0,4),s1)
            %errorr = errorr + 1
        %end
        %b=length(find(deocded(ctr+1)~=bitor(bitsll(s0,4),s1)))     %BER in BIT Detection
        %s0 = s0
        %bitsll_ = bitsll(s0,4)
        %s1 = s1
        %bitor_ = bitor(bitsll(s0,4),s1)
...
%errorr1 = length(find(s ~= deocded))
%errorr2 = find(s ~= deocded)
%errorr3 = s ~= deocded
end
