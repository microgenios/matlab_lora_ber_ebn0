function [signal_mod,signal,pld_swp, packet_hdr] = LoRa_Tx(message,Bandwidth,SF,Pt,Fs,df,varargin)
...
[packet, pld_swp, packet_hdr] = LoRa_Encode_Full(message,SF,CR) ; % encode message
...
function [packet, pld_swp,packet_hdr] = LoRa_Encode_Full(message,SF,CR)
...
%%len4=length(4*packet_hdr_gray)
%%len5=length(packet_pld_gray)
end
